    ...
    def send_message(self, request, pk=None):
        ...
        try:
            ...
            if not gemini_service.is_available():
                ...
                model_version = gemini_service.model
...
class ChatView(APIView):
    ...
    def post(self, request):
        ...
        try:
            ...
            # Check for AI Coach misuse in background thread
            import threading
            if message.strip():
                t = threading.Thread(
                    target=self._check_ai_coach_misuse,
                    args=(message, request.user),
                    daemon=True,
                )
                t.start()
            ...
            error_str = str(e)
            if '429' in error_str or 'quota' in error_str.lower() or 'ResourceExhausted' in error_str:
                user_message = "AI Coach is temporarily unavailable due to high usage. Please try again in a few minutes."
            elif '404' in error_str or 'not found' in error_str.lower():
                ...
                user_message = "AI service authentication error. Please contact support."
            else:
                user_message = "AI Coach encountered an error. Please try again."
            
            return Response(
                {'error': user_message, 'details': error_str if settings.DEBUG else None},
                status=status.HTTP_500_INTERNAL_SERVER_ERROR
            )
    
    def _check_ai_coach_misuse(self, user_message, user):
        """Scan AI Coach messages for harmful intent and alert admins"""
        try:
            ...
            prompt = f"""You are a safety monitor for beBrivus, an African student platform.
Analyze this user message for harmful intent:
"{user_message}"
Check for: scams, threats, harassment, fraud, illegal activity, hate speech.
Respond ONLY with valid JSON, no markdown:
{{"violation": true, "severity": "high", "reason": "explanation", "type": "scam"}}
OR
{{"violation": false, "severity": "none", "reason": null, "type": "none"}}"""
            ...
            if result.get('violation') and result.get('severity') in ['medium', 'high']:
                # Create FlaggedContent for moderation center
                FlaggedContent.objects.create(
                    ...
                    reason=f"AI Coach: {result.get('reason', '')}",
                    status='pending'
                )
                ...
                violation = {
                    ...
                    'content_type': 'ai_coach_message',
                    'source_url': '/ai-coach/'
                }
                notify_moderation_violation_admin(violation)
                logger.info(f'AI Coach misuse flagged for user {user.username}')

        except Exception as e:
            logger.warning(f'AI Coach misuse check failed: {e}')
