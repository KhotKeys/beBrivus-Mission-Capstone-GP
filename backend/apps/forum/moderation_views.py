...
class ModerationViewSet(viewsets.ViewSet):
    ...
    def list(self, request):
        ...
        flagged = FlaggedContent.objects.filter(status='pending')
        data = []
        for f in flagged:
            item = {
                'id': f.id,
            ...
            # Try to fetch discussion details including AI summary
            try:
    ...
    def audit_log(self, request):
        ...
        actions = ModerationAction.objects.select_related('admin_user', 'flagged_content')[:100]
        data = [{
            'id': a.id,
