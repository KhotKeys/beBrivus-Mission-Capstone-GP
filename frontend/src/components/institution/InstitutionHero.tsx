...
export const InstitutionHero: React.FC<InstitutionHeroProps> = ({ title, subtitle, badge, variant = 'default' }) => {
  const borderStyles = {
  ...
  return (
    <div className={`relative h-40 sm:h-48 mb-6 overflow-hidden ${borderStyles[variant]}`}>
      <div className="absolute inset-0 bg-gradient-to-br from-[#0d9488] via-[#14b8a6] to-[#2dd4bf] backdrop-blur-sm">
        <svg className="absolute inset-0 w-full h-full opacity-80" xmlns="http://www.w3.org/2000/svg">
          <defs>
            <pattern id="institution-lines" x="0" y="0" width="120" height="120" patternUnits="userSpaceOnUse">
          ...
          <rect width="100%" height="100%" fill="url(#institution-lines)" filter="url(#inst-blur)" />
        </svg>
