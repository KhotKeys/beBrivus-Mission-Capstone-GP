...
export const InstitutionApplicationResponsesPage: React.FC = () => {
  ...
  const updateStatusMutation = useMutation({
    mutationFn: async ({ id, status, feedback }: { id: number; status: string; feedback?: string }) => {
      console.log('Institution updating:', { id, status, feedback });
      const response = await api.patch(`/applications/${id}/`, { status, notes: feedback });
      console.log('Institution update response:', response.data);
      return response;
    },
    onSuccess: () => {
