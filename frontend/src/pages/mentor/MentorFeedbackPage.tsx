...
const CATEGORY_OPTIONS = [
  { value: 'session_issue',        label: 'Session / Booking Problem' },
  { value: 'student_behaviour',    label: 'Student Behaviour Report' },
  { value: 'payment_payout',       label: 'Payment / Payout Issue' },
  { value: 'profile_visibility',   label: 'Profile or Visibility Issue' },
...
const CATEGORY_COLORS: Record<string, { bg: string; color: string }> = {
  ...
  compliment:         { bg: '#e6f2f3', color: '#16a34a' },
...
export default function MentorFeedbackPage() {
  ...
  const [category, setCategory] = useState('session_issue');
      ...
      <div style={{ minHeight: '100vh', background: '#f9fafb', width: '100%', overflowX: 'hidden', boxSizing: 'border-box' }}>
        <style>{`
          * { box-sizing: border-box; }
          ...
          .feedback-left-col, .feedback-right-col {
            min-width: 0;
            width: 100%;
          }
          @media (max-width: 768px) {
            .feedback-hero-text { padding: 36px 16px !important; }
            .feedback-grid { grid-template-columns: 1fr; gap: 16px; }
            .feedback-right-col { position: static !important; }
          }
          @media (max-width: 480px) {
            .feedback-left-col > div, .feedback-right-col > div {
              padding: 16px !important;
              border-radius: 12px !important;
            }
            .feedback-grid { gap: 12px; }
          }
          .feedback-left-col input, .feedback-left-col select, .feedback-left-col textarea, .feedback-left-col button {
            max-width: 100%;
            width: 100%;
            box-sizing: border-box;
          }
          .feedback-right-col p, .feedback-right-col span {
            word-break: break-word;
            overflow-wrap: break-word;
          }
          .feedback-history-scroll::-webkit-scrollbar { width: 4px; }
          .feedback-history-scroll::-webkit-scrollbar-track { background: #f1f1f1; border-radius: 4px; }
          .feedback-history-scroll::-webkit-scrollbar-thumb { background: #d1d5db; border-radius: 4px; }
          .feedback-history-scroll::-webkit-scrollbar-thumb:hover { background: #9ca3af; }
        `}</style>
        ...
        <div style={{
          ...
          <div className="feedback-hero-text" style={{
            ...
            <p style={{
              ...
              Report session issues, billing problems, or share suggestions with the beBrivus team
            </p>
          </div>
        </div>
        ...
        <div style={{ maxWidth: '1100px', margin: '0 auto', padding: '0 16px 48px', boxSizing: 'border-box', width: '100%' }}>
          <div className="feedback-grid">
            <div className="feedback-left-col">
              ...
              {submitted ? (
                <div style={{ textAlign: 'center', padding: '40px 20px', width: '100%', boxSizing: 'border-box' }}>
                  ...
                  <h2 style={{ margin: '0 0 8px', fontSize: '22px', fontWeight: '700', color: '#111827' }}>Feedback Submitted!</h2>
                  <p style={{ color: '#6b7280', margin: '0 0 28px', fontSize: '15px', lineHeight: '1.6' }}>
                    Thank you! We will review your feedback and get back to you.
                  </p>
                  <button
                    onClick={resetForm}
                ...
                <div>
                  <h2 style={{ margin: '0 0 24px', fontSize: '20px', fontWeight: '700', color: '#111827' }}>New Feedback</h2>
                  <div style={{ marginBottom: '20px' }}>
                    ...
                    {rating > 0 && <p style={{ margin: '4px 0 0', fontSize: '12px', color: '#9ca3af' }}>{rating}/5 stars — click again to remove</p>}
                  </div>
                  <div style={{ marginBottom: '20px' }}>
                    <label style={{ display: 'block', fontWeight: '600', fontSize: '14px', color: '#374151', marginBottom: '8px' }}>Category</label>
                    <select
                      value={category}
                  ...
                  <div style={{ marginBottom: '20px' }}>
                    <label style={{ display: 'block', fontWeight: '600', fontSize: '14px', color: '#374151', marginBottom: '8px' }}>Subject</label>
                    <input
                      type="text"
                      value={subject}
                  ...
                  <div style={{ marginBottom: '28px' }}>
                    ...
                    <p style={{ margin: '4px 0 0', fontSize: '12px', color: message.length < 20 ? '#ef4444' : '#9ca3af' }}>
                      {message.length < 20 ? `${20 - message.length} more characters needed` : `${message.length} characters`}
                    </p>
                  </div>
                  <button
            ...
            <div className="feedback-right-col">
              ...
              <h2 style={{ margin: '0 0 20px', fontSize: '18px', fontWeight: '700', color: '#111827' }}>My Previous Feedback</h2>
              {historyLoading ? (
                ...
                <div style={{ textAlign: 'center', padding: '40px' }}>
                  ...
                  <p style={{ color: '#9ca3af', fontSize: '13px', margin: '4px 0 0' }}>Your submissions will appear here</p>
                </div>
              ) : (
                      ...
                      <div key={fb.id} style={{ background: '#f9fafb', borderRadius: '12px', padding: '14px 16px', marginBottom: '12px', border: '1px solid #e5e7eb', width: '100%', boxSizing: 'border-box', wordBreak: 'break-word', overflowWrap: 'break-word' }}>
                        ...
                        {fb.admin_note && ['resolved', 'closed'].includes(fb.status) && (
                          <div style={{ background: '#e6f2f3', borderLeft: '3px solid #125B66', borderRadius: '8px', padding: '10px 12px', marginTop: '10px' }}>
                            <p style={{ margin: '0 0 4px', fontSize: '11px', fontWeight: '700', color: '#15803d', textTransform: 'uppercase' }}>beBrivus Response</p>
